...
import type { ExplorerItem } from "../engine/types";
	...
	implements vscode.TreeDataProvider<ExplorerItem>
{
	...
	getTreeItem(element: ExplorerItem): vscode.TreeItem {
		...
		if (
			element.contextValue === "expression" ||
			element.contextValue === "context" ||
	...
	private loadSavedData(): void {
		...
		this.savedExpressions = expressions.map((expr, index) => ({
			...
			description:
				expr.expression.length > 30
					? expr.expression.substring(0, 27) + "..."
					: expr.expression,
