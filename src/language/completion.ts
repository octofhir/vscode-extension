...
export class CompletionProvider implements vscode.CompletionItemProvider {
	...
	async provideCompletionItems(
		...
		if (this.shouldShowProperties(beforeCursor, context)) {
			completionItems.push(
				...(await this.getPropertyCompletions(beforeCursor, currentWord)),
			);
		}
	...
	private shouldShowFunctions(
		beforeCursor: string,
		context: vscode.CompletionContext,
	): boolean {
		// Show functions after dot, opening parenthesis, or at the beginning
		return (
			/(\.|^|\s|\()$/.test(beforeCursor) || context.triggerCharacter === "."
		);
	}

	private shouldShowProperties(
		beforeCursor: string,
		context: vscode.CompletionContext,
	): boolean {
	...
	private getFunctionCompletions(currentWord: string): vscode.CompletionItem[] {
		...
		return functions.map((func) => {
			...
			item.documentation = new vscode.MarkdownString(
				this.engine.getFunctionDocumentation(func) || "",
			);
