...
export function activate(context: vscode.ExtensionContext) {
	...
	playgroundViewProvider = new PlaygroundViewProvider(
...
function registerCommands(context: vscode.ExtensionContext) {
			...
			try {
				...
				if (isValid) {
					vscode.window.showInformationMessage(
						"FHIRPath expression is valid. Hover over the expression for detailed information.",
					);
				} else {
					vscode.window.showInformationMessage(
						"FHIRPath expression has validation issues. Hover over the expression to see details and get help.",
					);
				}
			} catch (error) {
				vscode.window.showErrorMessage(
					`Validation error: ${error}. Hover over the expression for troubleshooting help.`,
				);
			}
		},
	);
	...
	const debugInfoCommand = vscode.commands.registerCommand(
		...
		() => {
			const debugInfo = {
				extensionVersion:
					vscode.extensions.getExtension("octofhir.fhirpath-extension")
			...
			vscode.window.showInformationMessage(
				"Debug information written to FHIRPath output channel",
			);
		},
	);
	...
	const showDocumentationCommand = vscode.commands.registerCommand(
		...
		() => {
			vscode.env.openExternal(
				vscode.Uri.parse("https://build.fhir.org/ig/HL7/FHIRPath/"),
			);
		},
	);
	...
	const playgroundEvaluateCommand = vscode.commands.registerCommand(
		...
		() => {
			...
			vscode.window.showInformationMessage(
				"Use the Evaluate button in the playground view",
			);
		},
	);
	...
	const playgroundExportCommand = vscode.commands.registerCommand(
		...
		() => {
			...
			vscode.window.showInformationMessage(
				"Use the Export button in the playground view",
			);
		},
	);
...
function registerViews(context: vscode.ExtensionContext) {
	...
	// Register AST view
	context.subscriptions.push(
		vscode.window.registerWebviewViewProvider("fhirpathAst", astViewProvider),
	);
...
async function loadContextFromFile() {
	...
	if (fileUri && fileUri[0]) {
		...
		try {
			...
			vscode.window.showInformationMessage(
				"FHIR resource context loaded from file",
			);
		} catch (error) {
...
async function loadContextFromServer() {
	...
	if (!serverUrl) {
		vscode.window.showErrorMessage(
			"No FHIR server URL configured. Please set fhirpath.server.url in settings.",
		);
		return;
	}
	...
	if (resourceId) {
		try {
			const resource = await fhirPathEngine.loadFromServer(
				serverUrl,
				resourceId,
			);
			fhirPathEngine.setContext(resource);
			vscode.window.showInformationMessage(
				`FHIR resource ${resourceId} loaded from server`,
			);
		} catch (error) {
			vscode.window.showErrorMessage(
				`Failed to load resource from server: ${error}`,
			);
		}
	}
}
...
async function loadContextManually() {
	...
	if (json) {
		try {
			...
			fhirPathEngine.setContext(resource);
			vscode.window.showInformationMessage(
				"FHIR resource context set manually",
			);
		} catch (error) {
